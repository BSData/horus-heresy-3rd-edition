...
Models  need to include the Charactaristic modifications for Prime Benefits that mod them (Copy from existing unit profile where applicable)
- For Models with the Command Sub-Type - Paragon of Battle - Increment WS, BS & A by 1 if Parent contains Paragon of Battle 
- For all Units that use the standard Profile, but not High Command or Unique - add the Combat Veterans Modifier Group to the profile
